...
1 - pchisq(chi2, df = df.residual(owls_p))  # Calculating the p-value
  ...
  # Note here that the Chi2 test is unilateral, so it only tests for overdispersion. However, underdispersion is less common and generaly
  ...
  # To adress this problem, we will rerun the model, but with a negative binomial distribution.
...
printCoefmat(quasi_table(owls_p), digits = 3)  # This applies the quasi_table function to your estimates calculated with 
  ...
  # as the quasi_table function only produces a table of the ajusted estimates, it may be easier to work with the glmmTMB function
...
ggplot(owls_pred, aes(x = 1:nrow(owls_pred), y = pred, ymin = q025, ymax = q975)) +
  ...
  geom_point(aes(y = SiblingNegotiation))  # Produces a graph with the intervals in gray, a line representing the simulated mean
                                           ...
                                           # majority of the dots, it seems too narrow in the left part of the graph and too large
                                           # in the right part.
...
sum(Owls$SiblingNegotiation == 0)  # The real number of zeros in your data.
  ...
  # indicates that the model doesn't simulate enough zeros. To have a model ajusted to your data, you need to consider this
...
ggplot(owls_pred, aes(x = 1:nrow(owls_pred), y = pred, ymin = q025, ymax = q975)) +
  ...
  geom_point(aes(y = SiblingNegotiation), alpha = 0.5)  # Produces a graph with the nb CI and mean in green, and the zinb CI
                                                        ...
                                                        # in the right part of the graph, and a narrower CI in the left part.
